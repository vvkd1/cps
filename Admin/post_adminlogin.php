		...
		if($count > 0) {
			...
			if($rowsetting->nooffailedpasswordattempt>0)
			{
				...
				if(count($row)>0)
				{
					...
					{
						$sql = "UPDATE tb_printadmin SET password_status = 0 WHERE userid = '".$_REQUEST['userid']."'";
						$db->query($sql);
					}
				}
			}			
			unset($sumbit);
			echo '{"error":"true", "htmlcontent":"Incorrect Userid or password or password expired."}';
			exit(0);
		}
	}
?>