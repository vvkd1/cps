...
// FUNCTIONS
    ...
    // set item
    function SetItem($url, $title, $description, $pubdate, $category=false, $commentslink=false , $content_encoded=false) {
		...
		$this->items[$this->nritems]['comments']=$commentslink;
    ...
    function Output() {
        ...
        for($k=0; $k<$this->nritems; $k++) {
			...
			$output .= '<comments>'.$this->items[$k]['comments'].'</comments>'."\n";
			$output .= '<pubDate>'.$this->items[$k]['pubDate'].'</pubDate>'."\n";
			$output .= '<category> '.$this->items[$k]['category'].' </category>'."\n";
			
			
            $output .= '<description><![CDATA['.$this->items[$k]['description'].']]></description>'."\n";
			$output .= '<dc:encoded><![CDATA['.$this->items[$k]['content_encoded'].']]></dc:encoded>'."\n";
