<?php require_once('global.php');
	if($_POST)
	{
		if($_REQUEST['txtpassword'] != "")
		{
			...
			if(count($result)>0)
			{
				if(checkLastPassword(md5($password),$_SESSION["admin_id"]))
				{
					$sql = "UPDATE tb_printadmin SET password = '".md5($password)."', is_temp_password = 1 WHERE adminid = '".$_SESSION['admin_id']."'";		
					$db->query($sql);
					
					$sqlpassword = "Insert into tb_cps_adminpasswords (adminid,password,date) values('". $_SESSION['admin_id'] ."','". md5($password) ."','". date("y-m-d") ."')";
