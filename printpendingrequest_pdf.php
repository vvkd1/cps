...
if(isset($_REQUEST['type']) && !empty($_REQUEST['type']) && $_REQUEST['type'] == 'search'  && 
	 ...
	 isset($_REQUEST['to']) && !empty($_REQUEST['to']) ) {
		$sql = "select * from tb_pending_print_req where cps_date between '".date('Y-m-d',strtotime($_REQUEST['frm']))."' and '".date('Y-m-d', strtotime($_REQUEST['to']))."' ";
} else {
	$sql = "select * from tb_pending_print_req";
}
...
$pdf->SetTitle('Pending Print Reports');
//$pdf->SetSubject('TCPDF Tutorial');
//$pdf->SetKeywords('TCPDF, PDF, example, test, guide');	

// set default header data
$pdf->SetHeaderData('', '', 'Pending Print Reports', '');
...
if($result = $db->get_results($sql)){
	...
	foreach($result as $row) {
							...
							$data .= '<tr>
                              ...
                              <td class="thwidthtd" align="center">'.$row->cps_act_jointname1.'</td>
